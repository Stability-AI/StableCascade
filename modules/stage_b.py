            ...
            if i > 0:
                self.down_downscalers.append(
                    nn.Sequential(
                        ...
                        nn.Conv2d(
                            c_hidden[i - 1], c_hidden[i], kernel_size=2, stride=2
                        ),
                    )
                )
            else:
            ...
            if i > 0:
                self.up_upscalers.append(
                    nn.Sequential(
                        ...
                        nn.ConvTranspose2d(
                            c_hidden[i], c_hidden[i - 1], kernel_size=2, stride=2
                        ),
                    )
                )
            else:
    ...
    def forward(self, x, r, effnet, clip, pixels=None, **kwargs):
        ...
        x = x + self.effnet_mapper(
            nn.functional.interpolate(
                effnet.float(), size=x.shape[-2:], mode="bilinear", align_corners=True
            )
        )
        x = x + nn.functional.interpolate(
            self.pixels_mapper(pixels).float(),
            size=x.shape[-2:],
            mode="bilinear",
            align_corners=True,
        )
