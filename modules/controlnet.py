...
from .cnet_modules.face_id.arcface import ArcFaceRecognizer, FaceDetector
        ...
        if bottleneck_mode == "effnet":
            ...
            if c_in != 3:
                ...
                self.backbone[0][0] = nn.Conv2d(
                    c_in, 24, kernel_size=3, stride=2, bias=False
                )
                if c_in > 3:
    ...
    def __call__(self, x):
        ...
        if self.resize is not None:
            x = nn.functional.interpolate(
                x, size=(self.resize, self.resize), mode="bilinear"
            )
        edges = [
            cv2.Canny(
                x[i].mul(255).permute(1, 2, 0).cpu().numpy().astype(np.uint8), 100, 200
            )
            for i in range(len(x))
        ]
        edges = torch.stack(
            [torch.tensor(e).div(255).unsqueeze(0) for e in edges], dim=0
        )
        if self.resize is not None:
...
class QRFilter(BaseFilter):
    def __init__(
        self, device, resize=224, blobify=True, dilation_kernels=None, blur_kernels=None
    ):
    ...
    def __call__(self, x):
        ...
        if self.blobify:
            ...
            if d_kernel > 0:
                blob_mask = (
                    (
                        (
                            torch.linspace(-0.5, 0.5, d_kernel).pow(2)[None]
                            + torch.linspace(-0.5, 0.5, d_kernel).pow(2)[:, None]
                        )
                        < 0.3
                    )
        ...
        # mask
        vmax, vmin = (
            x.amax(dim=[2, 3], keepdim=True)[0],
            x.amin(dim=[2, 3], keepdim=True)[0],
        )
        th = (vmax - vmin) * 0.33
...
class SRFilter(BaseFilter):
    ...
    def __call__(self, x):
        x = torch.nn.functional.interpolate(
            x.clone(), scale_factor=self.scale_factor, mode="nearest"
        )
        return torch.nn.functional.interpolate(
            x, scale_factor=1 / self.scale_factor, mode="nearest"
        )
...
class SREffnetFilter(BaseFilter):
    ...
    def __call__(self, x):
        ...
        with torch.no_grad():
            effnet_embedding = self.effnet(
                self.effnet_preprocess(x.to(self.device))
            ).cpu()
        effnet_embedding = torch.nn.functional.interpolate(
            effnet_embedding, scale_factor=1 / self.scale_factor, mode="nearest"
        )
        upscaled_image = torch.nn.functional.interpolate(
            x, scale_factor=1 / self.scale_factor, mode="nearest"
        )
...
class InpaintFilter(BaseFilter):
    def __init__(self, device, thresold=None, p_outpaint=0.4):
        ...
        self.saliency_model.load_state_dict(
            load_or_fail("modules/cnet_modules/inpainting/saliency_model.pt")
        )
    ...
    def __call__(self, x, mask=None, threshold=None, outpaint=None):
        ...
        if mask is None:
            ...
            if outpaint is None:
                ...
                saliency_map = ~saliency_map
            interpolated_saliency_map = torch.nn.functional.interpolate(
                saliency_map.float(), size=x.shape[2:], mode="nearest"
            )
            saliency_map = (
                torchvision.transforms.functional.gaussian_blur(
                    interpolated_saliency_map, 141
                )
                > 0.5
            )
            inpainted_images = torch.where(saliency_map, torch.ones_like(x), x)
            mask = torch.nn.functional.interpolate(
                saliency_map.float(), size=inpainted_images.shape[2:], mode="nearest"
            )
        else:
        ...
        for i in range(x.size(0)):
            ...
            for j in range(bounding_boxes.size(0)):
                if np.random.rand() > self.p_drop:
                    sx, sy, ex, ey = (
                        (bounding_boxes[j] / 32).clamp(min=0).round().int().tolist()
                    )
                    ...
                    visual_aid[
                        i, :, int(sy * 32) : int(ey * 32), int(sx * 32) : int(ex * 32)
                    ] += self.id_colors[j % 13, :, None, None]
                    visual_aid[
                        i, :, int(sy * 32) : int(ey * 32), int(sx * 32) : int(ex * 32)
                    ] *= 0.5
