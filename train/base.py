...
from core.data import Bucketeer, MultiFilter, setup_webdataset_path
...
class DataCore(WarpCore):
    ...
    def webdataset_path(self):
        ...
        if isinstance(
            self.config.webdataset_path, str
        ) and self.config.webdataset_path.strip().endswith(".yml"):
            with open(self.config.webdataset_path, "r", encoding="utf-8") as file:
                dataset_path = yaml.safe_load(file)
        return setup_webdataset_path(
            dataset_path, cache_path=f"{self.config.experiment_id}_webdataset_cache.yml"
        )
    ...
    def webdataset_preprocessors(self, extras: Extras):
        ...
        #     rules={
        #         ("old_caption", "caption"): lambda oc, c: get_caption(
        #             json.loads(oc)["og_caption"], c, p_og=0.05
        #         )
        #     }
        # )
    ...
    def setup_data(self, extras: Extras) -> WarpCore.Data:
        ...
        dataset = (
            ...
            .select(
                MultiFilter(
                    rules={f[0]: eval(f[1]) for f in self.config.dataset_filters}
                )
        ...
        return self.Data(
            dataset=dataset, dataloader=dataloader, iterator=dataloader_iterator
        )
    ...
    def get_conditions(
        ...
        captions, images = batch.get("captions"), batch.get("images")
        batch_size = len(captions)

        text_embeddings, text_pooled_embeddings, image_embeddings = None, None, None

        if any(field in return_fields for field in ["clip_text", "clip_text_pooled"]):
            captions_unpooled = self._prepare_captions_unpooled(
                captions, batch_size, is_eval, is_unconditional
            )
            clip_tokens_unpooled = self._tokenize_captions(models, captions_unpooled)
        ...
        if "clip_img" in return_fields:
            image_embeddings = self._prepare_image_embeddings(
                images,
                batch_size,
                models,
                extras,
    ...
    def _prepare_captions_unpooled(
        self, captions, batch_size, is_eval, is_unconditional
    ):
        if is_eval:
            return [""] * batch_size if is_unconditional else captions
        rand_idx = np.random.rand(batch_size) > 0.05
        return [str(c) if keep else "" for c, keep in zip(captions, rand_idx)]

    def _tokenize_captions(self, models, captions_unpooled):
    ...
    def _prepare_image_embeddings(
        ...
        if images is not None:
            ...
            if not is_eval and not is_unconditional and eval_image_embeds:
...
class TrainingCore(DataCore, WarpCore):
    ...
    @abstractmethod
    def forward_pass(
        self, data: WarpCore.Data, extras: WarpCore.Extras, models: Models
    ):
        ...
        for i in pbar:
            ...
            if models.generator_ema is not None and i % self.config.ema_iters == 0:
                update_weights_ema(
                    ...
                    models.generator,
                    beta=(
                        self.config.ema_beta if i > self.config.ema_start_iters else 0
                    ),
                )
            ...
            if (
                i == 1
                or i % (self.config.save_every * self.config.grad_accum_steps) == 0
                or i == max_iters
            ):
                ...
                if np.isnan(loss.mean().item()):
                    if self.is_main_node and self.config.wandb_project is not None:
                        tqdm.write(
                            "Skipping sampling & checkpoint because the loss is NaN"
                        )
                        wandb.alert(
    ...
    def save_checkpoints(self, models: Models, optimizers: Optimizers, suffix=None):
        ...
        if (
            suffix == ""
            and self.info.total_steps > 1
            and self.info.total_steps % self.config.backup_every == 0
        ):
            self.save_checkpoints(
                models, optimizers, suffix=f"_{self.info.total_steps // 1000}k"
            )
    ...
    def sample(self, models: Models, data: WarpCore.Data, extras: Extras):
        ...
        with torch.no_grad():
            ...
            noised, _, _, logSNR, noise_cond, _ = extras.gdf.diffuse(
            ...
            if self.is_main_node:
                noised_images = torch.cat(
                    [
                        self.decode_latents(noised[i : i + 1], batch, models, extras)
                ...
                pred_images = torch.cat(
                    [
                        self.decode_latents(pred[i : i + 1], batch, models, extras)
                ...
                sampled_images = torch.cat(
                    [
                        self.decode_latents(sampled[i : i + 1], batch, models, extras)
                ...
                sampled_images_ema = torch.cat(
                    [
                        self.decode_latents(
                            sampled_ema[i : i + 1], batch, models, extras
                        )
                ...
                torchvision.utils.save_image(
                    collage_img, f"{self.config.experiment_id}_latest_output.jpg"
                )
