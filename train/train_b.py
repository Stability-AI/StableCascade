        ...
        if images is not None:
            ...
            if is_eval and not is_unconditional:
                ...
                if not is_eval:
                    ...
                    if any(rand_idx):
                        effnet_embeddings[rand_idx] = models.effnet(
                            extras.effnet_preprocess(effnet_images[rand_idx])
                        )
        else:
    ...
    def _pyramid_noise(self, epsilon, size_range=None, levels=10, scale_mode="nearest"):
        ...
        for i in range(1, levels):
            ...
            if size_range is None or (
                ...
                or size_range[0] <= w <= size_range[1]
            ):
                offset = torch.randn(
                    epsilon.size(0), epsilon.size(1), h, w, device=self.device
                )
                epsilon = (
                    epsilon
                    + torch.nn.functional.interpolate(
                        offset, size=epsilon.shape[-2:], mode=scale_mode
                    )
                    * m
                )
