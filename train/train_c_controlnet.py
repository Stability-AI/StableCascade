...
class WurstCore(TrainingCore, DataCore, WarpCore):
    ...
    def setup_extras_pre(self) -> Extras:
        gdf = GDF(
            ...
            offset_noise=self.config.offset_noise
            if self.config.offset_noise is not None
            else 0.0,
        )
        sampling_configs = {
            "cfg": 5,
    ...
    def get_cnet(
        self, batch: dict, models: Models, extras: Extras, cnet_input=None, **kwargs
    ):
        images = batch["images"]
        with torch.no_grad():
            ...
            cnet_input, cnet_input_preview = (
                cnet_input.to(self.device),
    ...
    def setup_optimizers(self, extras: Extras, models: Models) -> Optimizers:
        optimizer = optim.AdamW(
            models.controlnet.parameters(), lr=self.config.lr
    ...
    def setup_schedulers(
        ...
        scheduler = GradualWarmupScheduler(
            optimizers.controlnet, multiplier=1, total_epoch=self.config.warmup_updates
        )
    ...
    def _extracted_from_backward_pass_11(
        ...
        loss_adjusted.backward()
        result = nn.utils.clip_grad_norm_(models.controlnet.parameters(), 1.0)
    ...
    def sample(self, models: Models, data: WarpCore.Data, extras: Extras):
        ...
        with torch.no_grad():
            ...
            conditions, unconditions = (
                {**conditions, "cnet": cnet},
                {**unconditions, "cnet": cnet},
            )
            ...
            if self.is_main_node:
                ...
                if images.size(-1) != noised_images.size(-1) or images.size(
                    ...
                    cnet_input = nn.functional.interpolate(
                        cnet_input, size=noised_images.shape[-2:], mode="bicubic"
                    )
