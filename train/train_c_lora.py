...
class WurstCore(TrainingCore, DataCore, WarpCore):
    ...
    def setup_models(self, extras: Extras) -> Models:
        ...
        for tkn_regex, aggr_regex in self.config.train_tokens:
            if (tkn_regex.startswith("[") and tkn_regex.endswith("]")) or (
                ...
                new_embedding = torch.zeros_like(
                    text_model.text_model.embeddings.token_embedding.weight.data
                )[:1]
                if (
                    aggr_regex is not None
                ):  # aggregate embeddings to provide an interesting baseline
                    aggr_tokens = [
                        v
                        for k, v in tokenizer.vocab.items()
                        if re.search(aggr_regex, k) is not None
                    ]
                    if aggr_tokens:
                        new_embedding = text_model.text_model.embeddings.token_embedding.weight.data[
                            aggr_tokens
                ...
                selected_tokens = [
                    v
                    for k, v in tokenizer.vocab.items()
                    if re.search(tkn_regex, k) is not None
                ]
        ...
        for module in generator.modules():
            if isinstance(module, LoRA) or (
                hasattr(module, "_fsdp_wrapped_module")
                and isinstance(module._fsdp_wrapped_module, LoRA)
            ):
        ...
        if self.config.lora_checkpoint_path is not None:
            ...
            lora.load_state_dict(
                ...
                else lora_checkpoint["state_dict"]
            )
    ...
    def setup_optimizers(self, extras: Extras, models: Models) -> Optimizers:
        optimizer = optim.AdamW(
            models.lora.parameters(), lr=self.config.lr
    ...
    def setup_schedulers(
        ...
        scheduler = GradualWarmupScheduler(
            optimizers.lora, multiplier=1, total_epoch=self.config.warmup_updates
        )
    ...
    def _extracted_from_backward_pass_11(
        ...
        loss_adjusted.backward()
        result = nn.utils.clip_grad_norm_(models.lora.parameters(), 1.0)
